#!/usr/bin/env bash
set -eu
if [[ ! -f custom-configs/config-user.properties ]] ; then
  ...
  cat >custom-configs/config-user.properties <<EOF
nis.bootName = $name
nis.bootKey = $key
EOF

echo -e "\e[32mThe config file generated is at custom-configs/config-user.properties\e[39m"
