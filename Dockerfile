...
# NEM software
RUN curl -L http://bob.nem.ninja/nis-0.6.93.tgz > nis-0.6.93.tgz

#RUN curl http://bob.nem.ninja/nis-0.6.93.tgz.sig > nis-0.6.93.tgz.sig
#RUN gpg --keyserver keys.gnupg.net --recv-key A46494A9
#RUN gpg --verify nis-0.6.93.tgz.sig nis-0.6.93.tgz

# New signature scheme, not always published
RUN sha=$(curl -L -s http://bigalice3.nem.ninja:7890/transaction/get?hash=$(curl -L -s  http://bob.nem.ninja/nis-0.6.93.tgz.sig | grep txId | sed -e 's/txId: //') | jq -r '.transaction.message.payload[10:74]') && \
    echo "$sha nis-0.6.93.tgz"  > /tmp/sum && \
    sha256sum -c /tmp/sum

RUN tar zxf nis-0.6.93.tgz
