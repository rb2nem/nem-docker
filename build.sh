...
docker exec -it nanowallet_build bash -c "cd /NanoWallet && [[ \"$(git rev-parse @{u})\" == \"$(git rev-parse @)\" ]] || ( echo 'updating code' && git pull && npm install && sed -i -e '/browserSync.init/,+12d' gulpfile.js && gulp build-app && checkout gulpfile.js ) "
